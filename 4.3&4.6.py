...
# Simulating AI capability growth with a saturation effect (e.g., using a logistic growth model)
C_data = 245 / (1 + np.exp(-0.5 * (time_data - 5)))  # Logistic growth, saturates around 700

# Simulating adoption rate influenced by AI capability, also showing a saturation effect
A_data = 0.9 / (1 + np.exp(-0.8 * (C_data / 50 - 3)))  # Adoption rate as a function of AI capability
...
def error_function(params):
    ...
    return error_C + 100000*error_A

# 初始参数猜测
initial_guess = [0.15, 500, 0.05] #内在增长率 r： 这个值反映AI能力在理想情况下的增长速度。最大能力 K,采纳率常数 β
...
C0, A0 = 53, 0.12
r_fit, K_fit, beta_fit = 0.5, 205.7549, 0.0010  # 最优参数估计
